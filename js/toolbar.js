		...
		$('.layertext').tooltip({ position: { my: "center bottom", at: "right+10 top+5" }, hide: 100, show: 500 });
			...
			buttons = ['#cameracontrols','#camera', '#helper', '#canvas', '#axis', '#wireframe', '#select', '#measure', '#objectedit', '#buffer', '#vertexedit']
			...
			$('#singleselect').click( function() {
				...
				if (singleselect === false) {
					...
					$('#mode').text("Current Pointer Mode: Selection");
					...
					$('#mode').text("Current Pointer Mode: Visualise");
			...
			$('#vertexedit').click( function() {
				...
				if (VERTEX == false && ACTION == false) {
					...
					$('#mode').text("Current Pointer Mode: Edit");
